...
def get_deleted_tokens_dir(user_id: int) -> Path:
    deleted_dir = get_user_dir(user_id) / "deleted_tokens"
    deleted_dir.mkdir(exist_ok=True)
    return deleted_dir


def get_existing_deleted_tokens_dir(user_id: int) -> Path:
    return BASE_DATA_DIR / str(user_id) / "deleted_tokens"


def get_deleted_manifest_path(user_id: int) -> Path:
    return get_deleted_tokens_dir(user_id) / "deletions.json"


def get_existing_deleted_manifest_path(user_id: int) -> Path:
    return BASE_DATA_DIR / str(user_id) / "deleted_tokens" / "deletions.json"
...
def load_deleted_batches(user_id: int) -> list[dict]:
    path = get_existing_deleted_manifest_path(user_id)
    if not path.exists():
        return []
    ...
    try:
        ...
        logging.warning("Could not read deleted tokens manifest for user %s", user_id)
        return []

    return data if isinstance(data, list) else []


def save_deleted_batches(user_id: int, batches: list[dict]):
    with open(get_deleted_manifest_path(user_id), "w", encoding="utf-8") as file:
        json.dump(batches, file, ensure_ascii=False, indent=2)


def get_deleted_token_files(user_id: int) -> list[Path]:
    deleted_dir = get_existing_deleted_tokens_dir(user_id)
    if not deleted_dir.exists():
        return []

    files: list[Path] = []
    for path in deleted_dir.rglob("*"):
        if path.is_file() and path.name != "deletions.json":
            files.append(path)
    return sorted(files, key=lambda item: item.stat().st_mtime, reverse=True)


def get_deleted_tokens_summary(user_id: int) -> dict:
    batches = load_deleted_batches(user_id)
    files = get_deleted_token_files(user_id)
    total_deleted = sum(int(batch.get("count", 0)) for batch in batches)
    last_deleted_at = batches[0]["deleted_at"] if batches else None
    return {
        "batches": batches,
        "batch_count": len(batches),
        "token_count": total_deleted if total_deleted else len(files),
        "last_deleted_at": last_deleted_at,
        "files": files,
    }


def archive_deleted_tokens(user_id: int) -> dict:
    account_files = get_account_files(user_id)
    if not account_files:
        return {"count": 0, "batch": None}

    deleted_root = get_deleted_tokens_dir(user_id)
    deleted_at = datetime.now().isoformat(timespec="seconds")
    batch_id = datetime.now().strftime("%Y%m%d_%H%M%S")
    batch_dir = deleted_root / batch_id
    ...
    for file_path in account_files:
        ...
        if target_path.exists():
            target_path = batch_dir / f"{file_path.stem}_{int(datetime.now().timestamp())}{file_path.suffix}"
    ...
    save_deleted_batches(user_id, batches)
    return {"count": len(moved_files), "batch": batch_record}
...
def admin_user_card_kb(user_id: int, page: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [make_button("🗃 Удаленные токены", f"admin_deleted_{user_id}_{page}", "danger")],
            [
...
def admin_user_deleted_kb(user_id: int, page: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [make_button("📦 Выгрузить удаленные токены", f"admin_deleted_export_{user_id}_{page}", "primary")],
            [
                make_button("⬅️ Карточка", f"admin_user_{user_id}_{page}", "primary"),
                make_button("👥 Пользователи", f"admin_users_page_{page}", "primary"),
            ],
...
def get_admin_user_card_text(user_id: int) -> str:
    ...
    stats = snapshot["stats"]
    deleted_summary = get_deleted_tokens_summary(user_id)
    ...
    return (
        ...
        f"• <b>Удаленных токенов:</b> <code>{deleted_summary['token_count']}</code>\n"
        f"• <b>Всего сохранений:</b> <code>{stats['total']}</code>\n"
        f"• <b>Размер данных:</b> <code>{format_size(snapshot['total_size'])}</code>\n"
...
def get_admin_deleted_tokens_text(user_id: int) -> str:
    summary = get_deleted_tokens_summary(user_id)
    if not summary["batches"]:
        return (
            "<b>Удаленные токены</b>\n\n"
            f"• <b>ID пользователя:</b> <code>{user_id}</code>\n"
            "• Удаленных токенов пока нет."
        )

    batches_preview = "\n".join(
        f"• <code>{format_dt(batch.get('deleted_at'))}</code> — удалено <code>{batch.get('count', 0)}</code>"
        for batch in summary["batches"][:10]
    )
    return (
        "<b>Удаленные токены</b>\n\n"
        f"• <b>ID пользователя:</b> <code>{user_id}</code>\n"
        f"• <b>Всего удаленных токенов:</b> <code>{summary['token_count']}</code>\n"
        f"• <b>Удалений:</b> <code>{summary['batch_count']}</code>\n"
        f"• <b>Последнее удаление:</b> <code>{format_dt(summary['last_deleted_at'])}</code>\n\n"
        "<b>История удалений</b>\n"
        f"{batches_preview}"
    )
...
def create_deleted_tokens_archive(user_id: int) -> Path | None:
    deleted_dir = get_existing_deleted_tokens_dir(user_id)
    if not deleted_dir.exists():
        return None
    ...
    for path in deleted_dir.rglob("*"):
        if path.is_file():
            sources.append((path, str(path.relative_to(deleted_dir.parent)).replace("\\", "/")))

    if not sources:
        return None

    zip_path = Path(f"deleted_tokens_{user_id}_{int(datetime.now().timestamp())}.zip")
    if create_zip_archive(zip_path, sources):
        return zip_path
    if zip_path.exists():
        zip_path.unlink()
...
def is_playwright_browser_missing(error: Exception) -> bool:
    ...
    return "Executable doesn't exist" in error_text or "Please run the following command to download new browsers" in error_text


async def run_login_process_safe(user_id: int, message: types.Message, state: FSMContext):
    try:
        await login_process(user_id, message, state)
    except Exception as exc:
        logging.error("Unhandled error in login_process task: %s", exc)
        if is_playwright_browser_missing(exc):
            await message.answer(
                "<b>На сервере не установлен браузер Playwright.</b>\n"
                "Выполните на хосте:\n"
                "<code>python -m playwright install chromium</code>\n\n"
                "Если это Linux-хост и браузер все равно не стартует, выполните еще:\n"
                "<code>python -m playwright install-deps chromium</code>",
                parse_mode="HTML",
            )
        else:
            await message.answer(
                "<b>Ошибка при запуске браузера для получения QR-кода.</b>",
                parse_mode="HTML",
            )
...
async def extract_phone_from_settings(page: Page) -> str | None:
    try:
        ...
        click_targets = [
            ...
            "text=Настройки",
            "text=Settings",
            "button[aria-label='Параметры']",
            "button[aria-label='Настройки']",
            "button[aria-label='Settings']",
            "a[aria-label='Параметры']",
            "a[aria-label='Настройки']",
            "a[aria-label='Settings']",
            "[data-testid='settings-button']",
            ".settings-btn",
            ".icon-settings",
            "button:has-text('Параметры')",
            "button:has-text('Настройки')",
            "button:has-text('Settings')",
            "a:has-text('Параметры')",
        ...
        if not clicked:
            logging.warning("Could not click any settings button")
            return None
        ...
        try:
            ...
            if body_text:
                match = re.search(r"(\+7|8)\s*\(?\d{3}\)?\s*\d{3}[-\s]?\d{2}[-\s]?\d{2}", body_text)
                if match:
                    phone = match.group(0)
                    phone = re.sub(r"[+\s\-\(\)]", "", phone)
                    if phone.startswith("8"):
...
async def extract_account_data(page: Page) -> dict | None:
    ...
    try:
        ...
        phone_value = auth_json.get("phone") or auth_json.get("msisdn") or auth_json.get("login")
        if phone_value:
            normalized_phone = re.sub(r"[^\d]", "", str(phone_value))
            if len(normalized_phone) >= 11:
                return {"phone": normalized_phone, "device_id": device_id, "auth_data": auth_data}
    except Exception:
        pass

    phone = await extract_phone_from_settings(page)
    if phone:
    ...
    try:
        ...
        if viewer_id:
            return {"phone": f"id{viewer_id}", "device_id": device_id, "auth_data": auth_data}
    except Exception:
        pass

    return {"phone": "unknown", "device_id": device_id, "auth_data": auth_data}
...
async def handle_login(callback: CallbackQuery, state: FSMContext):
    ...
    asyncio.create_task(run_login_process_safe(user_id, callback.message, state))
...
async def clear_confirm_second(callback: CallbackQuery, state: FSMContext):
    ...
    archive_result = archive_deleted_tokens(user_id)
    count = archive_result["count"]
    deleted_at = archive_result["batch"]["deleted_at"] if archive_result["batch"] else None
...
@dp.callback_query(F.data.startswith("admin_deleted_"))
async def admin_deleted_router(callback: CallbackQuery):
    touch_user_profile(callback.from_user)
    if not is_main_admin(callback.from_user.id):
        await deny_admin_access(None, callback)
        return
    ...
    if parts[2] == "export":
        ...
        page = int(parts[4]) if len(parts) > 4 else 0
        await callback.answer()
        zip_path = create_deleted_tokens_archive(user_id)
        if not zip_path:
            await callback.message.answer(
                f"У пользователя <code>{user_id}</code> нет удаленных токенов для выгрузки.",
                parse_mode="HTML",
            )
            return
        ...
        try:
            await bot.send_document(
                callback.from_user.id,
                FSInputFile(zip_path),
                caption=f"Удаленные токены пользователя {user_id}.",
            )
            await callback.message.answer(
                f"Архив удаленных токенов пользователя <code>{user_id}</code> отправлен.",
                parse_mode="HTML",
            )
        finally:
            if zip_path.exists():
    ...
    await callback.answer()
    await callback.message.edit_text(
        get_admin_deleted_tokens_text(user_id),
        reply_markup=admin_user_deleted_kb(user_id, page),
        parse_mode="HTML",
    )
